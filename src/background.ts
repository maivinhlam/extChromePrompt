...
const FLOW_WORKFLOWS_API_URL_PATTERN = 'https://aisandbox-pa.googleapis.com/v1/flowWorkflows/*';
const FLOW_RENAME_API_BASE_URL = 'https://aisandbox-pa.googleapis.com/v1/flowWorkflows';
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === 'DOWNLOAD_URL') {
    ...
    void chrome.downloads
      ...
      .catch((error: Error) => {
        if (pendingDirectDownload) {
          const pendingIndex = pendingDirectDownloads.indexOf(pendingDirectDownload);
          if (pendingIndex >= 0) {
  ...
  if (message?.type === 'GET_FLOW_REQUEST_HEADERS') {
    ...
    const headers = typeof tabId === 'number' ? latestFlowApiHeadersByTab.get(tabId) : null;
  ...
  if (message?.type === 'PATCH_FLOW_WORKFLOW_DISPLAY_NAME') {
    void patchFlowWorkflowDisplayName(sender, message as FlowWorkflowRenameMessage)
...
function normalizeMouseButton(button?: DebuggerMouseButton): DebuggerMouseButton {
...
async function focusSenderTab(sender: chrome.runtime.MessageSender): Promise<void> {
  const tabId = sender.tab?.id;
...
async function patchFlowWorkflowDisplayName(
  ...
  const response = await fetch(`${FLOW_RENAME_API_BASE_URL}/${encodeURIComponent(workflowId)}`, {
    method: 'PATCH',
    headers: {
      accept: '*/*',
