...
async function withPageInteractionLock<T>(action: () => Promise<T>): Promise<T> {
...
async function debuggerClickElement(element: HTMLElement, button: DebuggerMouseButton = 'left'): Promise<boolean> {
...
export async function waitForTileDoneById(
  ...
  { status: 'completed'; tile: HTMLElement } | { status: 'failed' } | { status: 'timeout' } | { status: 'stopped' }
> {
  ...
  return withPageInteractionLock(async () => {
    if (mediaContainer) {
      ...
      if (name) {
        ...
        await appendAutomationLog(`Could not extract image name for ${imageName} from media container.`);
      }
