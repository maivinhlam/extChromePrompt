...
export function findPromptInput(): HTMLElement | null {
  const preferredSlateEditors = Array.from(
    document.querySelectorAll("[role='textbox'][contenteditable='true'][data-slate-editor='true']")
  ) as HTMLElement[];
  ...
  const placeholderMatchedEditor = preferredSlateEditors.find((editor) => {
    ...
    const placeholder =
      editor.querySelector("[data-slate-placeholder='true']")?.textContent?.trim().toLowerCase() || '';
...
export async function waitForDialog(timeoutMs: number): Promise<HTMLElement | null> {
...
export async function waitForMenu(timeoutMs: number): Promise<HTMLElement | null> {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
...
export function findImgItemByAlt(text: string, root: ParentNode = document): HTMLElement | null {
  const candidates = Array.from(root.querySelectorAll(`img[alt='${text}']`)) as HTMLElement[];
  ...
  for (const el of candidates) {
    ...
    const clickableParent = el.closest("button, [role='button'], [data-index], li, div") as HTMLElement | null;
...
export function findButtonByText(terms: string[], root: ParentNode = document): HTMLElement | null {
  const buttons = Array.from(root.querySelectorAll("button, [role='button'], [role='menuitem']")) as HTMLElement[];
  return (
    buttons.find((button) => {
      if (!isVisible(button)) {
        return false;
