...
export async function test(pendingRenameTasks: Set<Promise<void>>) {
  ...
  const renameTask = (async (): Promise<void> => {
    ...
    if (!newTileId) {
      await appendAutomationLog(`Rename skipped for '${renameTo}': no new tile detected in time.`);
      return;
    }
    ...
    if (tileResult.status !== 'completed') {
      await appendAutomationLog(`Rename skipped for '${renameTo}': tile did not reach 100% in time.`);
      return;
    }
    ...
    if (state.mode === 'video') {
      ...
      if (downloaded) {
        ...
        await appendAutomationLog(`Download skipped for '${renameTo}': API request or menu flow failed.`);
      }
    }
  })().catch(async (error: unknown) => {
    await appendAutomationLog(`Rename task failed for '${renameTo}': ${(error as Error).message}`);
  });

  pendingRenameTasks.add(renameTask);

  if (pendingRenameTasks.size) {
    await appendAutomationLog(`Waiting for ${pendingRenameTasks.size} rename task(s) to finish.`);
