import type { SceneNumbers } from '../domain/automation-types';

export function parseSceneNumbers(prompt: string, fallback: number): SceneNumbers {
...
export function formatSceneName(sceneNumber: number, typeLabel: string, itemNumber?: number): string {
  void itemNumber;
  return `SCENE ${sceneNumber}` + (typeLabel === '' ? '' : ` - ${typeLabel} `);
}

export function extractPromptPrefixName(prompt: string, fallback: string): string {
  const text = String(prompt || '').trim();
...
export function setInputValue(node: HTMLElement, value: string): void {
  ...
  const slateStrings = Array.from(node.querySelectorAll("span[data-slate-string='true']")) as HTMLElement[];
